...
#include "firmware/cyanfw_sd_mmc_rel_csp_silicon.h"
#include "firmware/cyanfw_mtp_sd_mmc_rel_csp_silicon.h"
#endif

#ifdef WESTBRIDGE_ANTIOCH
#include "firmware/cyanfw_sd_mmc_rel_silicon.h"
#include "firmware/cyanfw_mtp_sd_mmc_rel_silicon.h"
#endif
...
static cyasdevice *cy_as_device_controller = NULL;
int cy_as_device_init_done;
const char *dev_handle_name = "cy_astoria_dev_handle" ;

static int firmware_number = 0;
...
static void cy_wq_function (struct work_struct *work)
{
	...
	{
		cy_as_hal_print_message(KERN_ERR"- - - - - - - - - - - - -   [cyasdevice.c] %s: SD card is removed.\n", __func__) ;
		if( status )
		{
			cy_as_hal_print_message(KERN_ERR"[cyasdevice.c] %s: cyasblkdev_blk_exit\n", __func__) ;
			down(&cy_as_device_controller->wq_sema);
	...
	{
		cy_as_hal_print_message(KERN_ERR"[cyasdevice.c] %s: leave standby\n", __func__) ;
		down(&cy_as_device_controller->wq_sema);
		cyasdevice_leave_standby();
		up(&cy_as_device_controller->wq_sema);
		...
		cy_as_hal_print_message(KERN_ERR"++++++++++++  [cyasdevice.c] %s: SD card is inserted.\n", __func__) ;
		if(cy_as_device_controller->cy_work->f_reload == 2)
		{
			...
			if( ret == 1 )
			{
				cy_as_hal_print_message("[cyasdevice.c] %s: cyasblkdev_blk_exit\n", __func__) ;
				cyasblkdev_blk_exit();
			...
			{
				cyasblkdev_blk_exit();
				cy_as_hal_print_message("[cyasdevice.c] %s: reload firmware\n", __func__) ;
				status = cy_as_device_controller->cy_work->f_status = 0x0;
			}
			else
			{
...
static ssize_t show_cyas_diagnostics(struct device *dev, struct device_attribute *attr, char *buf) 
{
	return snprintf(buf, PAGE_SIZE, "%s\n", cyas_diagnostics);
}

static ssize_t store_cyas_diagnostics(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int	ret;
	int	mode = 0;
	int i=0, len;
...
static DEVICE_ATTR(cyas_diagnostics, S_IRUGO | S_IWUSR, show_cyas_diagnostics, store_cyas_diagnostics);

static int serial;
static ssize_t show_cyas_serial(struct device *dev, struct device_attribute *attr, char *buf)
{
	serial = cyasblkdev_get_serial();
	return snprintf(buf, PAGE_SIZE, "0x%08x\n", serial);
}

static ssize_t store_cyas_serial(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	return strlen(buf);
}
...
static ssize_t store_cyas_wbcid(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	return strlen(buf);
}
...
static void cyasdevice_deinit(cyasdevice *cy_as_dev)
{
	...
	if (cy_as_dev->hal_tag) {
		...
		if (stop_qc_kernel(dev_handle_name, cy_as_dev->hal_tag) != 0)
			cy_as_hal_print_message("<1>_cy_as_device: stopping QC Kernel HAL failed\n");
#endif

#ifdef CONFIG_MACH_C110_WESTBRIDGE_AST_CRAM_HAL
		if (cy_as_hal_c110_cram_stop(dev_handle_name,	cy_as_dev->hal_tag) != 0)
...
static int cyasdevice_initialize(void)
{
...
#ifdef CONFIG_MACH_QC_WESTBRIDGE_AST_HAL
 /* start QC HAL init instsnce */
	if (!start_qc_kernel(dev_handle_name,	&(cy_as_dev->hal_tag), cy_false)) {
		cy_as_hal_print_message("<1>_cy_as_device: start QC HAL failed\n") ;
		goto done;
	}
#endif
 ...
 /* start C110 HAL init instsnce */
	if (!cy_as_hal_c110_cram_start(dev_handle_name,	&(cy_as_dev->hal_tag), cy_false)) {
 ...
 /* start C110 HAL init instsnce */
	...
	if (IS_ERR(westbridge_pd)) {
		cy_as_hal_print_message("[%s] error in register the platform driver for west bridge \n", __FUNCTION__);
		platform_driver_unregister(&west_bridge_driver);
	...
	else
	{
		...
		if (ret)
			cy_as_hal_print_message("[%s] error in device_create_file for device_create_file 1\n", __FUNCTION__);
		ret = device_create_file(&westbridge_pd->dev, &dev_attr_serial);
		if (ret)
			cy_as_hal_print_message("[%s] error in device_create_file for device_create_file 2\n", __FUNCTION__);
	...
	{
		cy_as_hal_print_message("<1>_cy_as_device: firmware_number=0, download firmware cyanfw_sd_mmc_rel_csp_silicon_array image\n") ;
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyanfw_sd_mmc_rel_csp_silicon_array.fw_image, CYANFW_SD_MMC_REL_CSP_SILICON_SIZE , 0, 0) ;
	}
	else
	{
		cy_as_hal_print_message("<1>_cy_as_device: firmware_number=1, download firmware cyanfw_mtp_sd_mmc_rel_csp_silicon_array image\n") ;
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyanfw_mtp_sd_mmc_rel_csp_silicon_array.fw_image, CYANFW_MTP_SD_MMC_REL_CSP_SILICON_SIZE , 0, 0) ;
	}
#endif
#ifdef WESTBRIDGE_ANTIOCH
	if( firmware_number == 0)
	{
		cy_as_hal_print_message("<1>_cy_as_device: firmware_number=0, download firmware cyanfw_sd_mmc_rel_silicon_array image\n") ;
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyanfw_sd_mmc_rel_silicon_array.fw_image, CYANFW_SD_MMC_REL_SILICON_SIZE , 0, 0) ;
	}
	else
	{
		cy_as_hal_print_message("<1>_cy_as_device: firmware_number=1, download firmware cyanfw_mtp_sd_mmc_rel_silicon_array image\n") ;
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyanfw_mtp_sd_mmc_rel_silicon_array.fw_image, CYANFW_MTP_SD_MMC_REL_SILICON_SIZE , 0, 0) ;
	}
#endif
#ifdef WESTBRIDGE_ASTORIA
	if( firmware_number == 0)
	{
		...
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyastfw_sd_mmc_rel_silicon_array.fw_image, CYASTFW_SD_MMC_REL_SILICON_SIZE , 0, 0) ;
	}
	else
	{
...
static int 	g_first_error_enter = 0;
static int 	g_first_error_leave = 0;
...
int cyasdevice_enter_standby(void)
{
	...
	if (f_isStandby == cy_false)
	{
		...
		if( g_first_error_enter || g_first_error_leave )
			cy_as_hal_print_message(KERN_ERR"[%s] 1st error %d - %d \n", __func__, g_first_error_enter, g_first_error_leave) ;
	}
	return 0;
}
	...
	if (f_isStandby == cy_true)
	{
		...
		while(count)
		{
			...
			if((v1 & CY_AS_MEM_RST_RSTCMPT) == 0)
				cy_as_hal_print_message(KERN_ERR"Initialization Message Received,but reset complete bit still not set\n");
			else
			{
				v2=cy_as_hal_read_register(cy_as_device_controller->hal_tag, CY_AS_MEM_P0_VM_SET);
				if(v2 & CY_AS_MEM_P0_VM_SET_CFGMODE)
					cy_as_hal_print_message(KERN_ERR"Initialization Message Received,but config bit still not set\n");
				else
					break;
			}
			count--;
...
int cyasdevice_reload_firmware(int mtp_mode)
{
...
#else
	ret = cy_as_misc_reset(cy_as_dev->dev_handle, cy_as_reset_hard, cy_true, 0, 0);
	if (ret != CY_AS_ERROR_SUCCESS) {
		cy_as_hal_print_message( "<1>_cy_as_device: cy_as_misc_reset failed. reason code: %d\n", ret) ;
	}
	...
	if( firmware_number == 0)
	{
		cy_as_hal_print_message("<r>reload_fw: firmware_number=0, download firmware cyanfw_sd_mmc_rel_csp_silicon_array image\n") ;
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyanfw_sd_mmc_rel_csp_silicon_array.fw_image, CYANFW_SD_MMC_REL_CSP_SILICON_SIZE , 0, 0) ;
	}
	else
	{
		cy_as_hal_print_message("<r>reload_fw: firmware_number=0, download firmware cyanfw_mtp_sd_mmc_rel_csp_silicon_array image\n") ;
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyanfw_mtp_sd_mmc_rel_csp_silicon_array.fw_image, CYANFW_MTP_SD_MMC_REL_CSP_SILICON_SIZE , 0, 0) ;
	}
#endif
#ifdef WESTBRIDGE_ANTIOCH
	if( firmware_number == 0)
	{
		cy_as_hal_print_message("<r>reload_fw: firmware_number=0, download firmware cyanfw_sd_mmc_rel_silicon_array image\n") ;
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyanfw_sd_mmc_rel_silicon_array.fw_image, CYANFW_SD_MMC_REL_SILICON_SIZE , 0, 0) ;
	}
	else
	{
		cy_as_hal_print_message("<r>reload_fw: firmware_number=0, download firmware cyanfw_mtp_sd_mmc_rel_silicon_array image\n") ;
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyanfw_mtp_sd_mmc_rel_silicon_array.fw_image, CYANFW_MTP_SD_MMC_REL_SILICON_SIZE , 0, 0) ;
	}
#endif
#ifdef WESTBRIDGE_ASTORIA
	if( firmware_number == 0)
	{
		cy_as_hal_print_message("<r>reload_fw: firmware_number=0, download firmware cyastfw_sd_mmc_rel_silicon_array image\n") ;
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyastfw_sd_mmc_rel_silicon_array.fw_image, CYASTFW_SD_MMC_REL_SILICON_SIZE , 0, 0) ;
	}
	else
	{
		cy_as_hal_print_message("<r>reload_fw: firmware_number=0, download firmware cyastfw_mtp_sd_mmc_rel_silicon_array image\n") ;
		ret = cy_as_misc_download_firmware( cy_as_dev->dev_handle, cyastfw_mtp_sd_mmc_rel_silicon_array.fw_image, CYASTFW_MTP_SD_MMC_REL_SILICON_SIZE , 0, 0) ;
	}
#endif
...
static int cyasdevice_sleep_thread(void *d)
{
	...
	#ifndef WESTBRIDGE_NDEBUG
	cy_as_hal_print_message(KERN_ERR"%s:%x started, cyasdevice:%p\n", __func__, qth_pid, cy_as_device_controller) ;
	#endif
	...
	do {
		...
		if( cy_as_device_controller->thread_sleep_enable )
		{
			if(cy_as_device_controller->thread_sleep_flags & CYASDEVICE_THREAD_TIMER)
			{
				if(!ret)  //timeout
				{
					...
					if( check_retry > 3 )
					{
						cy_as_device_controller->thread_sleep_flags = CYASDEVICE_THREAD_SUSPENDED;
						#ifdef WESTBRIDGE_NDEBUG
						cy_as_hal_print_message(KERN_ERR"%s: chage mode to CYASDEVICE_THREAD_SUSPENDED\n", __func__);
						#endif
					}
				}
				else  // wake_up
			...
			else
			{
				...
				{
					#ifdef WESTBRIDGE_NDEBUG
					cy_as_hal_print_message(KERN_ERR"%s: chage mode to CYASDEVICE_THREAD_TIMER\n", __func__);
					#endif
		...
		else
		{
			check_retry = 0;
			#ifdef WESTBRIDGE_NDEBUG
			cy_as_hal_print_message(KERN_ERR"%s: CYASDEVICE_THREAD diabled\n", __func__);
			#endif
			cy_as_device_controller->thread_sleep_flags = CYASDEVICE_THREAD_SUSPENDED;
		}
...
static int __init cyasdevice_init(void)
{
...
#ifdef __USE_CYAS_AUTO_SUSPEND__
	{
		...
		init_completion(&cy_as_device_controller->thread_sleep_complete);
		...
		ret = kernel_thread(cyasdevice_sleep_thread, cy_as_device_controller, CLONE_KERNEL);
		if (ret >= 0) {
	...
	{
		cy_as_device_controller->cy_work = (cy_work_t *)kmalloc(sizeof(cy_work_t), GFP_KERNEL);
		if(cy_as_device_controller->cy_work)
		{
			INIT_DELAYED_WORK( &cy_as_device_controller->cy_work->work, cy_wq_function );
			cy_as_device_controller->cy_work->f_status = 0x0;
			cy_as_device_controller->cy_work->f_reload = 1;
			cy_as_device_controller->cy_work->f_isrunning = 0;

#ifdef __USE_ISR_FOR_SD_DETECT__
			cy_as_hal_configure_sd_isr((void *)cy_as_device_controller->hal_tag, (irq_handler_t) cyasdevice_int_handler);
#endif
			 if( cy_as_hal_detect_SD() )
			{
			    cy_as_hal_print_message("<1>%s: SD card scan:\n", __func__) ;
			    //queue_delayed_work( cy_as_device_controller->cy_wq, &cy_as_device_controller->cy_work->work, 10 * HZ);
			}
			else
			{
				queue_delayed_work( cy_as_device_controller->cy_wq, &cy_as_device_controller->cy_work->work, HZ);
			}
