...
if [ ! -f /system/.agat/cwmmanager3-installed ];
then
  if [ "$payload_extracted" == "0" ];then
    extract_payload
  fi
  ...
  mkdir /system/.agat
  chmod 755 /system/.agat
  echo 1 > /system/.agat/cwmmanager3-installed
fi
...
#if [ "${romtype}a" == "0a" ];
#then
#if [ ! -f /system/.agat/liblights-installed ];then
  ...
  echo 1 > /system/.agat/liblights-installed
#fi
#fi
